...
make_pca_ggplot <- function(pca_object, # PCA object generated by the rda() function from vegan package
  ...
  pca_site_scores <- scores(pca_object, display = "sites", choices = component_choices, scaling = scaling_choice)
  pca_specie_scores <- scores(pca_object, display = "species", choices = component_choices,scaling = scaling_choice)
  ...
  for (myvar in unique(c(discrete_variables, continuous_variables))){
    ...
    if (!is.null(continuous_variables) & myvar %in% continuous_variables){
      ...
      # TODO check for existing continuous colour?
    ...
    } else if (!is.null(discrete_variables) & myvar %in% discrete_variables){
  ...
  # component_choice_name_1 <- paste0("PC",component_choices[1])
  # component_choice_name_2 <- paste0("PC",component_choices[2])
  if (show_x_label){
  ...
  if (!is.null(continuous_variables)){
    ...
    en = envfit(pca_object, metadata.df[,continuous_variables,drop =F], permutations = 999, na.rm = TRUE, scaling = 0)
  ...
  if (!is.null(discrete_variables)){
    en = envfit(pca_object, metadata.df[,discrete_variables,drop =F], permutations = 999, na.rm = TRUE, scaling = 0)
    ...
    # Remove non-significant results
    unique_groups_discrete_variables.v <- metadata.df[,discrete_variables,drop =F][,names(en$factors$pvals),drop =F] %>% pivot_longer(., cols = everything()) %>% select(value) %>% unique() %>% pull()
  ...
  if (!is.null(species_to_include.v)){
  ...
  # Shape
  # FIXME if colour/shapre are not the same as shape, shape hould be hollow form and not coloured??
  if (use_shapes == T & !is.null(shape_variable)){
    if (!is.null(colour_variable) & !is.null(fill_variable)){
      myplot <-
        myplot +
        geom_point(aes_string(colour = colour_variable, fill = fill_variable, shape = shape_variable), size = point_size, stroke = point_line_thickness) +
      ...
      myplot <-
        myplot +
        geom_point(aes_string(colour = colour_variable, shape = shape_variable), fill = "black", size = point_size, stroke = point_line_thickness) +
        scale_shape_manual(values = shape_lists[[shape_variable]], name = shape_variable)
    } else if (is.null(colour_variable) & !is.null(fill_variable)){
      myplot <-
        myplot +
        geom_point(aes_string(fill = fill_variable, shape = shape_variable), colour = "black", size = point_size, stroke = point_line_thickness) +
        scale_shape_manual(values = shape_lists[[shape_variable]], name = shape_variable)
    }
  } else {
    myplot <-
      myplot +
      geom_point(
        aes_string(colour = colour_variable,fill = fill_variable, size = point_size),
        shape = default_shape, size = point_size, stroke = point_line_thickness
      )
  }
  ...
  # Colour
  if (all(!is.null(colour_variable) && colour_variable %in% continuous_variables)){
  ...
  } else if (!is.null(colour_variable)) {
  ...
  # Fill
  if (all(!is.null(fill_variable) && fill_variable %in% continuous_variables)){
  ...
  if (plot_hulls == T & !is.null(hull_variable)){
    ...
    myplot <-
      ...
      geom_polygon(data = hull_data.df, aes_string(fill = hull_variable, colour = hull_variable),show.legend = FALSE)
  ...
  if (plot_envfit_arrows == T & !is.null(en_coord_cont) && dim(en_coord_cont)[1] != 0){
    ...
    myplot <-
      ...
      geom_segment(data = en_coord_cont, aes_string(x = 0, y = 0,
                   ...
                   size = envfit_arrow_thickness,
                   alpha = envfit_arrow_alpha)
    if (envfit_arrow_label_type == "text"){
      myplot <-
        myplot +
        geom_text(data = en_coord_cont, aes_string(x = component_choice_name_1, y = component_choice_name_2),
  ...
  if (plot_specie_arrows == T && dim(pca_specie_data.df)[1] != 0 ){
    myplot <-
      myplot +
      ...
      if (specie_arrow_label_type == "text"){
        myplot <-
          ...
          geom_text(data = pca_specie_data.df,aes_string(x = component_choice_name_1, y = component_choice_name_2, label = "Species_label"),
                    ...
                    colour = specie_arrow_label_colour,check_overlap = T,
                    # position = position_dodge(0.9),
                    nudge_x = 0, nudge_y = 0
          )
