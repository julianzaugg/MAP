...
make_pca_ggplot <- function(pca_object, # PCA object generated by the rda() function from vegan package
  ...
  if (use_shapes == T & !is.null(shape_variable)){
    myplot <-
      myplot +
      geom_point(aes_string(colour = colour_variable, fill = fill_variable, shape = shape_variable), size = point_size) +
    ...
    myplot <-
      ...
      geom_point(
        ...
        shape = default_shape, size = point_size
      )
  }
  ...
  if (plot_hulls == T & !is.null(hull_variable)){
    hull_data.df <- pca_data.df %>%
      dplyr::group_by(get(hull_variable)) %>%
  ...
  if (plot_ellipses == T & !is.null(ellipse_variable)){
    ...
    rslt <- vegan::ordiellipse(pca_object,
                               ...
                               display = "sites",
                               choices=component_choices, kind = "ehull", draw = "none", label = F)
  ...
  if (plot_spiders == T & !is.null(spider_variable)){
    spider_data.df <-
      ...
      dplyr::group_by(get(spider_variable)) %>%
